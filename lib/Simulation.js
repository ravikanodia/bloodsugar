...
const Simulation = function(foodLookup, exerciseLookup, activityArray) {
  ...
  function advanceSimulation() {
    ...
    nextState.currentActivities = _.filter(
        activityArray,
        function(activity) {
          return activity.startingTimestamp <= timestamp &&
              activity.endingTimestamp > timestamp;
        });
        ...
        _.first(
            _.pluck(
                _.filter(activityArray, function(activity) {
                  return activity.startingTimestamp > that.state.timestamp;
                }),
                'startingTimestamp'));
 
    // Get soonest endingTimestamp
    var nextActivityEndTimestamp = _.first(
        _.pluck(that.state.currentActivities, 'endingTimestamp').sort());
  ...
  function outputState() {
    ...
    _.each(
        _.difference(
          ...
          states.length > 1 ? states[states.length - 2] : initialState),
        function(activity) {
