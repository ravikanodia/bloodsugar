...
const Simulation = function(foodLookup, exerciseLookup, activityArray) {
  ...
  function getNextSimulationTimestamp() {
    ...
    var restingIntercept = MyMath.nextIntercept(
        that.state.timestamp,
        that.state.glucose,
        getCurrentGlucoseDelta() / Constants.MILLIS_PER_HOUR,
        Constants.RESTING_GLUCOSE);

    // Get timestamp when (if ever) glucose will cross glycation threshold.
    var glycationIntercept = MyMath.nextIntercept(
        that.state.timestamp,
        that.state.glucose,
        getCurrentGlucoseDelta() / Constants.MILLIS_PER_HOUR,
        Constants.GLYCATION_THRESHOLD);

    // Ignore null and undefined values.
    var potentialTimestamps = _.filter(
        [nextActivityStartTimestamp, nextActivityEndTimestamp,
         restingIntercept, glycationIntercept],
         function(timestamp) {
           return !_.isUndefined(timestamp) && timestamp !== null; 
         });
  ...
  // Quick little algebra here
  function computeNextIntercept(
      currentValue, threshold, delta, currentTimestamp) {
    if (delta === 0) {
      return undefined;
    }
    var crossingTimestamp = currentTimestamp +
      (threshold - currentValue) * (Constants.MILLIS_PER_HOUR / delta);
    return (crossingTimestamp > currentTimestamp) ?
      crossingTimestamp : undefined;
  }
