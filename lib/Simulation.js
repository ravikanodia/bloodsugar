...
const Simulation = function(foodLookup, exerciseLookup, eventsArray) {
  ...
  function isDone() {
    return state.timestamp >= eventsArray[eventsArray.length - 1].timestamp;
  }

  // TODO: this also needs a lot of work.
  function advanceSimulation() {
    state.eventIndex = state.eventIndex + 1;
    state.timestamp = eventsArray[state.eventIndex].timestamp;
  }
